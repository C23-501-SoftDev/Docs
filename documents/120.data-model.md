...
erDiagram
    ...
    SPACE ||--o{ REQUIREMENT_NUMBER_COUNTER : "нумерует требования"
    
    TEMPLATE ||--o{ DOCUMENT : "используется в"
    TEMPLATE ||--o{ REQUIREMENT_NUMBER_COUNTER : "нумерует требования"
    ...
    DOCUMENT {
        bigint id PK
        string title
        string git_file_path "Относительный путь к .md-файлу в Git-репозитории (например, spaces/1/overview.md)"
...
| `is_deleted` | `BOOLEAN` | NOT NULL, DEFAULT FALSE | Флаг мягкого удаления. Установленный в `true` означает, что пользователь мягко удалён — не может войти в систему, исключён из списков и поиска, но физически остаётся в БД для сохранения истории авторства |
...
- `UNIQUE (login)`
- `UNIQUE (email)`
- `idx_users_not_deleted` на `(id) WHERE is_deleted = false`
...
- Комбинация `Guest` + `is_admin = true` означает "чистого администратора" — он может управлять пользователями и правами, но не имеет глобального доступа к контенту.
...
- Пользователи поддерживают soft-удаление через поле `is_deleted`. При `is_deleted = true` пользователь не может войти в систему, исключён из списков и результатов поиска, но физически остаётся в БД. Все связанные записи (права на пространства, memberships в группах, авторство документов) сохраняются для истории и аудита.
- При soft-удалении пользователя его `login` и `email` остаются зарезервированными, чтобы предотвратить повторную регистрацию под теми же данными.
...
| `is_deleted` | `BOOLEAN` | NOT NULL, DEFAULT FALSE | Флаг мягкого удаления. Установленный в `true` означает, что пространство мягко удалено — исключено из списков и поиска, но физически остаётся в БД для сохранения целостности документов и прав доступа |
| `created_at` | `TIMESTAMP` | DEFAULT CURRENT_TIMESTAMP | Дата и время создания |
| `updated_at` | `TIMESTAMP` | DEFAULT CURRENT_TIMESTAMP | Дата и время последнего обновления |
...
**Примечания:**
- Пространства поддерживают soft-удаление через поле `is_deleted`. При `is_deleted = true` пространство исключено из списков и результатов поиска, но физически остаётся в БД. Все документы пространства при этом автоматически архивируются (переводятся в статус `Deleted`). Связанные записи (права доступа) сохраняются для истории и аудита.
- При soft-удалении пространства его `name` остаётся зарезервированным, чтобы предотвратить повторное создание пространства с тем же именем.
- При мягком удалении пользователя-владельца пространства его права владельца автоматически передаются системному администратору (ID=1).
...
Основная таблица контента. **Хранит только метаданные**, в то время как фактическое содержимое документа в формате Markdown хранится исключительно в Git-репозитории. Связь с файлом осуществляется через поле `git_file_path`. Права доступа к документам определяются исключительно через права на уровне пространства и глобальную роль пользователя.

| Столбец | Тип | Ограничения | Описание |
| :--- | :--- | :--- | :--- |
| `id` | `BIGSERIAL` | PRIMARY KEY | Уникальный идентификатор документа |
| `title` | `VARCHAR(500)` | NOT NULL | Заголовок документа |
| `git_file_path` | `VARCHAR(500)` | NOT NULL, UNIQUE | Относительный путь к `.md`-файлу в Git-репозитории. Это единственный источник контента документа. |
| `status` | `VARCHAR(20)` | NOT NULL, DEFAULT 'Draft', CHECK (status IN ('Draft', 'Published', 'Deleted')) | Статус жизненного цикла. |
...
| `parent_document_id` | `BIGINT` | FOREIGN KEY → `documents(id)` ON DELETE RESTRICT | Родительский документ |
| `created_at` | `TIMESTAMP` | DEFAULT CURRENT_TIMESTAMP | Дата создания |
| `updated_at` | `TIMESTAMP` | DEFAULT CURRENT_TIMESTAMP | Дата последнего изменения (включая изменение статуса). |

**Индексы:**
- `PRIMARY KEY (id)`
- `UNIQUE (git_file_path)`
...
**Примечания:**
- Путь `git_file_path` может генерироваться автоматически при создании документа на основе его `id` и названия, что исключает конфликты имен.
...
- Права на чтение, редактирование и удаление документа определяются правами пользователя на пространство, которому принадлежит документ (`space_id`), и глобальной ролью пользователя.
- **Ограничения вложенности:** Дочерний документ обязан принадлежать тому же пространству, что и родительский (`space_id` совпадают). Проверка выполняется на уровне приложения при установке `parent_document_id`.
- **Удаление при наличии дочерних:**
  - Soft-удаление (перевод в `Deleted`) разрешено: дочерние документы автоматически переподчиняются родителю удаляемого документа.
  - Hard-delete запрещён, если есть дочерние документы в любом статусе (включая `Deleted`).
  - При нарушении (hard-delete) система возвращает ошибку с сообщением: «Невозможно удалить документ: у него есть дочерние документы. Сначала удалите или переместите их.»
- Связь `ON DELETE RESTRICT` на `parent_document_id` дополнительно защищает от случайного физического удаления родителя напрямую через БД.
...
- `idx_versions_created_at` на `(created_at)`
...
### 11. `requirement_number_counters` — Счётчики автонумерации требований

Хранит атомарные счётчики для генерации порядковых номеров требований внутри документов. Каждая комбинация `(space_id, template_id)` имеет свой независимый счётчик. Номер выделяется через `SELECT ... FOR UPDATE` для потокобезопасности.

| Столбец | Тип | Ограничения | Описание |
| :--- | :--- | :--- | :--- |
| `id` | `BIGSERIAL` | PRIMARY KEY | Уникальный идентификатор счётчика |
| `space_id` | `BIGINT` | NOT NULL, FOREIGN KEY → `spaces(id)` ON DELETE CASCADE | Пространство |
| `template_id` | `BIGINT` | NOT NULL, FOREIGN KEY → `templates(id)` ON DELETE CASCADE | Шаблон (тип документа) |
| `next_number` | `INTEGER` | NOT NULL, DEFAULT 1 | Следующий порядковый номер |
| `created_at` | `TIMESTAMP` | DEFAULT CURRENT_TIMESTAMP | Дата и время создания |
| `updated_at` | `TIMESTAMP` | DEFAULT CURRENT_TIMESTAMP | Дата и время последнего обновления |

**Ограничения:**
- `UNIQUE(space_id, template_id)` — один счётчик на пару пространство + шаблон.

**Индексы:**
- `PRIMARY KEY (id)`
- `idx_requirement_number_counters_space_template` на `(space_id, template_id)`

**Примечания:**
- Счётчик инкрементируется атомарно внутри транзакции с блокировкой строки (`FOR UPDATE`), что гарантирует уникальность номера даже при параллельных запросах.
- При первом запросе для новой комбинации `(space_id, template_id)` счётчик автоматически создаётся со значением `1`.
- При удалении пространства или шаблона счётчики удаляются каскадно.
...
| `spaces` | `documents` | `space_id` | `RESTRICT` | Нельзя удалить пространство с документами (физическое); soft-удаление архивирует все документы пространства |
...
| `documents` | `documents` | `parent_document_id` | `RESTRICT` | Нельзя физически удалить родителя, пока есть дочерние документы |
...
| `spaces` | `requirement_number_counters` | `space_id` | `CASCADE` | Счётчики не нужны без пространства |
| `templates` | `requirement_number_counters` | `template_id` | `CASCADE` | Счётчики не нужны без шаблона |