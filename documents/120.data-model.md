...
**Примечания:**
- Пространства поддерживают soft-удаление через поле `is_deleted`. При `is_deleted = true` пространство исключено из списков и результатов поиска, но физически остаётся в БД. Все связанные записи (документы, права доступа) сохраняются для истории и аудита.
...
| `spaces` | `documents` | `space_id` | `RESTRICT` | Нельзя удалить пространство с документами (физическое); soft-удаление не проверяет наличие активных документов |
