...
- **Удаление при наличии дочерних:**
  - Soft-удаление (перевод в `Deleted`) запрещено, если есть дочерние документы в статусах `Draft` или `Published`.
  - Hard-delete запрещён, если есть дочерние документы в любом статусе (включая `Deleted`).
  - При нарушении система возвращает ошибку с сообщением: «Невозможно удалить документ: у него есть дочерние документы. Сначала удалите или переместите их.»
