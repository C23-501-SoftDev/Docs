...
| `is_admin` | `BOOLEAN` | NOT NULL, DEFAULT FALSE | Флаг администратора. Дает доступ к панели администратора, управлению пользователями, пространствами и правами доступа |
...
Обеспечивает разграничение прав доступа на уровне пространств между пользователями.
...
**Индексы:**
- `PRIMARY KEY (id)`
- `idx_space_permissions_space` на `(space_id)`
- `idx_space_permissions_user` на `(user_id)`

---

### 4. `templates` — Шаблоны документов
...
**Индексы:**
- `PRIMARY KEY (id)`
- `idx_documents_space_status` на `(space_id, status) WHERE is_deleted = FALSE`
- `idx_documents_not_deleted` на `(is_deleted) WHERE is_deleted = FALSE`
...
### 8. `document_permissions` — Права доступа на уровне документа
...
| `users` | `space_permissions` | `user_id` | `CASCADE` | Права не нужны без пользователя |
...
| `spaces` | `space_permissions` | `space_id` | `CASCADE` | Права не нужны без пространства |
