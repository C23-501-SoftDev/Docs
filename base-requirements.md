...
* **Безопасность:** Разграничение прав доступа на уровне глобальных ролей, пространств, документов и пользователей.
...
| Роль | Описание |
| :--- | :--- |
| **GUEST** | Базовая роль. Дает только возможность входа в систему. Все права на контент получает через явное назначение прав на пространства (`SpacePermission`) и документы (`DocumentPermission`). |
...
3. **Управление пространствами (Spaces):**
    ...
    * Назначение ответственных редакторов и прав доступа (`READ`, `WRITE`, `OWNER`) для пользователей -> Сохранение.
...
graph TD
    ...
    CreateSpace --> SpaceRights[Назначение прав пользователей]
