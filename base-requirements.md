...
* **Навигация:** Система навигации по документам через оглавление и списки. При выборе пространства в иерархическом дереве в центральной части страницы (называемой «Последние документы») должны отображаться документы, относящиеся только к последнему выбранному пространству, с указанием его названия.
...
3. **Удаление (Soft-delete):** Поиск документа -> Проверка наличия дочерних документов в статусах Draft или Published. Если есть — они автоматически переподчиняются родителю удаляемого документа -> Пометка документа как удаленного (смена статуса) -> Рассылка уведомлений.
...
* **Управление пространствами:** Создание и настройка пространств (Spaces), назначение ответственных редакторов (только администраторы) и прав доступа для пользователей и групп на уровне всего пространства. Владельцем пространства может быть только пользователь с правами администратора (`is_admin = true`).
* **Hard-delete:** Возможность физического удаления документа из системы. Hard-delete блокируется, если у документа есть дочерние документы в любом статусе (включая Deleted).
* **Ограничение удаления:** Запрещено удалять системного пользователя (ID=1).
* **Передача владения:** При удалении пользователя, владеющего пространствами, права владельца этих пространств автоматически переходят к системному пользователю (ID=1).
...
* **Удаление:** Возможность удаления документа пользователем с правами редактора. При удалении родительского документа с дочерними элементами они автоматически переподчиняются родителю удаляемого документа.
...
1. **Черновик (Draft):** Статус по умолчанию при создании документа.
2. **Опубликован (Published):** Документ доступен для чтения всем пользователям, имеющим права доступа к соответствующему пространству. Статус `Published` назначается автором при создании или редактировании документа.
...
* При удалении документа, имеющего дочерние элементы, они автоматически переподчиняются родителю удаляемого документа (без блокировки удаления).
* Администратор или Редактор (если есть права) может восстановить документ, вернув его в статус "Опубликован" или "Черновик".
* При восстановлении документа, чей исходный родитель был удалён, восстанавливаемый документ переподчиняется "дедушке" (родителю своего удалённого родителя). При восстановлении пространства родительские связи сохраняются без изменений.
* Восстановление документа невозможно, если пространство, к которому он относится, было удалено.
* Физическое удаление (Hard-delete) доступно только Администратору. Hard-delete блокируется, если у документа есть дочерние документы в любом статусе (включая Deleted).
...
5. **Дата и время:** Во всей системе должно быть отображение даты и времени в едином корректном формате.

### Технические требования

1. **Стек технологий:** 
   - **Бэкенд:** Java 17+, Spring Boot, Spring Security, Spring Data JPA, Maven.
   - **Фронтенд:** Серверный рендеринг с Thymeleaf, JavaScript для интерактивных компонентов (включая редактор Markdown с предпросмотром на основе marked.js).
