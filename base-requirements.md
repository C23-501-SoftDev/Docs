...
1. **Авторизация:** Вход под учетной записью с ролью `EDITOR` (или с правами `WRITE` на пространство для `GUEST`).
...
2. **Управление пользователями:**
    ...
    * Создание новой учетной записи (с указанием роли `GUEST`/`READER`/`EDITOR` и флага `is_admin`).
    * Настройка и изменение ролей и флага `is_admin` -> Сохранение -> Рассылка уведомлений.
    * Блокировка пользователя (смена роли на `GUEST`, сброс `is_admin`).
3. **Управление пространствами (Spaces):**
    * Создание и настройка параметров пространства.
    * Назначение ответственных редакторов и прав доступа (`READ`, `WRITE`, `OWNER`) для пользователей и групп -> Сохранение.
...
graph TD
    ...
    AddUser --> UserRights[Настройка роли и флага is_admin]
    ...
    CreateSpace --> SpaceRights[Назначение прав пользователей и групп]
...
* **Создание:** Пользователь с ролью `EDITOR` или с правом `WRITE`/`OWNER` в пространстве должен иметь возможность создать документ, заполнив необходимые атрибуты.
...
* **Разграничение прав (UI):** Система должна скрывать элементы интерфейса (кнопки "Редактировать", "Удалить") от пользователей, не имеющих соответствующих прав на пространство, которому принадлежит документ.
...
1. **Черновик (Draft):** Начальный статус. Документ доступен только автору и администраторам (пользователям с `is_admin = true`). Создается при выборе шаблона или создании пустого документа.
...
* При удалении документа редактором, он помечается как удаленный (смена флага/статуса), но не стирается из БД. Удаление невозможно, если у документа есть дочерние документы в статусах Draft или Published.
* Администратор (пользователь с `is_admin = true`) или редактор (пользователь с ролью `EDITOR` или правом `WRITE`/`OWNER`) может восстановить документ из "Корзины", вернув его в статус "Опубликован".
* Физическое удаление (Hard-delete) доступно только пользователям с `is_admin = true` или происходит автоматически по истечении заданного периода удержания (опционально для MVP). Hard-delete блокируется, если у документа есть дочерние документы в любом статусе.
